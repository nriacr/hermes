...
SETTINGS_CSS = """
:root { color-scheme:dark; --bg:#121118; --panel:#1B1A25; --card:#1F1E2A; --line:#2E2C3D; --text:#F1EFFA; --muted:#A5A1C0; --accent:#4B47D6; --accent2:#6FE3A9; --ok:#6FE3A9; --bad:#FF93A6; --ink-faint:#726F8C; --surface:#1B1A25; --surface-2:#201F2C; --indigo-ink:#9C97FF; --indigo-soft:rgba(156,151,255,.14); --mint-ink:#6FE3A9; --mint-soft:rgba(111,227,169,.12); --rose-ink:#FF93A6; --rose-soft:rgba(255,147,166,.12); }
...
main { max-width:1080px; margin:0 auto; padding:28px 18px 44px; } .hero { border:1px solid var(--line); border-radius:20px; padding:22px; background:var(--panel); box-shadow:0 18px 42px rgba(0,0,0,.35); }
h1 { margin:0 0 8px; padding-left:50px; min-height:38px; display:flex; align-items:center; position:relative; color:var(--text); font-size:30px; font-weight:800; letter-spacing:-.03em; } h1::before { content:''; position:absolute; left:0; top:0; width:38px; height:38px; border-radius:12px; background:linear-gradient(155deg,#4B47D6,#1F8F66); } h2 { margin:24px 0 10px; font-size:18px; color:var(--text); } p { margin:0; color:var(--muted); line-height:1.5; font-size:13px; }
...
.button.primary { color:#fff; background:linear-gradient(135deg,#4B47D6,#3B37B0); } .button.secondary { color:var(--text); background:var(--surface-2); border-color:var(--line); } .button.danger { color:#fff5f7; background:var(--rose-ink); border-color:#e8a4ac; } .button.danger:hover { background:#ff7690; } .button.is-submitting { pointer-events:none; opacity:.7; }
.notice { margin:14px 0; padding:11px 13px; border-radius:12px; font-weight:700; font-size:13px; } .notice-ok { color:var(--mint-ink); background:var(--mint-soft); border:1px solid rgba(111,227,169,.35); } .notice-fail { color:var(--rose-ink); background:var(--rose-soft); border:1px solid rgba(255,147,166,.35); }
.settings-section { margin-top:18px; border:1px solid var(--line); border-radius:20px; padding:16px; background:var(--card); } details { border:1px solid var(--line); border-radius:14px; background:var(--surface-2); margin:9px 0; overflow:hidden; } summary { cursor:pointer; padding:13px 14px; font-weight:900; color:var(--text); list-style:none; } summary::-webkit-details-marker { display:none; } summary::before { content:'\u25b8'; display:inline-block; margin-right:8px; color:var(--ink-faint); } details[open] summary::before { transform:rotate(90deg); } .watch-tools { display:grid; gap:12px; } .watch-search { display:grid; gap:6px; max-width:440px; margin:0; color:var(--muted); font-size:12px; font-weight:750; } .watch-search input { width:100%; min-height:40px; border:1px solid var(--line); border-radius:11px; padding:10px 11px; background:var(--surface); color:var(--text); font:inherit; } .watch-group-filters { display:flex; flex-wrap:wrap; gap:8px; margin:0; } .watch-group-filter { min-height:34px; border:1px solid var(--line); border-radius:999px; padding:0 12px; background:var(--surface-2); color:var(--text); font:700 12px inherit; cursor:pointer; } .watch-group-filter[aria-pressed='false'] { color:var(--muted); background:transparent; opacity:.6; text-decoration:line-through; } .watch-group-filter:hover { border-color:#5f5adf; } .watch-actions { display:flex; align-items:center; gap:8px; min-height:40px; }
.saving-overlay { position:fixed; inset:0; z-index:20; display:grid; place-items:center; padding:20px; background:rgba(0,0,0,.6); backdrop-filter:blur(5px); } .saving-overlay[hidden] { display:none; } .saving-dialog { width:min(100%,430px); border:1px solid var(--line); border-radius:18px; padding:22px; background:var(--card); box-shadow:0 22px 50px rgba(0,0,0,.5); } .saving-dialog h2 { margin:0 0 9px; font-size:20px; } .saving-dialog p { font-size:14px; } .saving-spinner { width:28px; height:28px; margin:0 0 14px; border:4px solid rgba(156,151,255,.18); border-top-color:var(--indigo-ink); border-radius:50%; animation:hermes-spin .8s linear infinite; } @keyframes hermes-spin { to { transform:rotate(360deg); } }
.form-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:12px; padding:0 14px 14px; } label { display:grid; gap:6px; color:var(--muted); font-size:12px; font-weight:700; } input[type='text'], input[type='number'], input[type='url'], select, textarea { width:100%; min-height:40px; border-radius:11px; border:1px solid var(--line); background:var(--surface); color:var(--text); padding:10px 11px; font-size:13px; font-family:inherit; } textarea { resize:vertical; line-height:1.35; }
.checkbox-row { display:flex; align-items:center; gap:9px; min-height:40px; color:var(--text); } .danger { color:var(--rose-ink); } .footer-note { margin-top:14px; border-left:4px solid var(--ink-faint); padding:12px 14px; background:var(--surface-2); border-radius:10px; font-size:13px; }
...
(() => {
  ...
  if (window.location.hash === '#new-watch') {
    const newWatchSection = document.getElementById('new-watch');
    const newWatchDetails = newWatchSection?.querySelector('details');
    if (newWatchDetails) newWatchDetails.open = true;
    newWatchSection?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }
})();
""".strip()
...
def _new_watch_section(groups, known_titles=None):
    return (
        "<section id='new-watch' class='settings-section'><form method='post' action='./settings/save' data-settings-save>"
