...
SETTINGS_CSS = """
:root { color-scheme:dark; --bg:#111315; --panel:#1a1c1f; --card:#24272b; --line:#3c4147; --text:#f5f7fa; --muted:#b0b6be; --accent:#e4e5e3; --accent2:#b8bbba; --ok:#86dfb7; --bad:#ff9caf; }
...
def _watch_form(item, index, is_new=False, groups=None, known_titles=None):
    ...
    include_warehouse = False if is_new else item.get("include_warehouse", False)
...
def _build_watch(form, index):
    ...
    item = {
        ...
        "include_warehouse": _bool_from_form(form, prefix + "include_warehouse"),
