...
def _extract_supported_url(text: str) -> str:
    ...
    for match in URL_PATTERN.finditer(str(text or "")):
        ...
        try:
            ...
            resolved_url = _resolve_amazon_short_url(candidate)
            if resolved_url:
                return resolved_url
    return ""


def _resolve_amazon_short_url(url: str) -> str:
    """Resolve the amzn.eu links generated by Amazon's mobile share sheet once."""
    if urlparse(url).netloc.casefold() != "amzn.eu":
        return ""
    try:
        ...
        log(f"Telegram Amazon kısa linki açılamadı: {exc}")
        return ""
