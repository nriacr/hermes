...
def _render_table_row(row):
    ...
    priority_label = {"high": "Yüksek", "medium": "Orta", "low": "Düşük"}.get(priority, "")
    priority_dot = (
        f'<i class="priority-dot priority-{escape(priority)}" title="{priority_label} öncelik" '
        f'aria-label="{priority_label} öncelik"></i>'
        if not is_warehouse and priority in {"high", "medium", "low"}
        else ""
    )
    ...
    if product_url:
        label = (
            ...
            f'<span class="product-title">{priority_dot}{warehouse_tag}{product_title}</span></a>'
        )
    else:
        label = f'<span class="product-title">{priority_dot}{warehouse_tag}{product_title}</span>'
    ...
    return (
        ...
        f'<td data-label="Güncel fiyat" class="price-cell">{price}</td>'
        f'<td data-label="Hedef fiyat" class="target-cell">{target}</td>'
    ...
    return f"""
        <div class="table-wrap">
          <table class="price-summary-table">
            <thead><tr><th>Satıcı</th><th>Ürün Adı</th><th>Güncel<br>fiyat</th><th>Hedef<br>fiyat</th><th>Fark</th><th>Min / Maks</th><th>Son<br>güncelleme</th></tr></thead>
            <tbody>{body}</tbody>
          </table>
        </div>
    """
